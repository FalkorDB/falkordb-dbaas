...
resource "google_workflows_workflow" "disk_threshold_exceeded" {
  name        = "falkordb-instance-disk-threshold-exceeded"
  description = "Workflow to handle disk space threshold exceeded alerts from GKE"
  project     = var.project_id
  region      = var.region
...
main:
  ...
  steps:
    ...
    # 7. Iterate through users and send notification emails
    - send_emails_to_users:
        for:
          value: user
          in: $${users}
