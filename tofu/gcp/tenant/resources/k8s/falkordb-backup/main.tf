          ...
          spec {
            ...
            container {
              ...
              command = [
                "/bin/bash",
                "-c",
                "apt-get update && apt-get install -y kubectl redis-cli && kubectl exec  falkordb-redis-node-0 -n ${var.deployment_namespace} -- redis-cli -a '${var.falkordb_password}' BGSAVE && kubectl cp falkordb-redis-node-0:/data/dump.rdb dump.rdb -c redis --namespace ${var.deployment_namespace} && gsutil cp /tmp/dump.rdb gs://${var.backup_bucket_name}/${var.tenant_name}/dump_$(date +%Y-%m-%d-%H-%M-%S).rdb"
              ]
