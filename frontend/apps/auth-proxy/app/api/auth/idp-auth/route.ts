  ...
  if (authRequestPayload) {
    try {
      ...
      return NextResponse.redirect(saasDomainURL + "/grafana", {
        status: 302,
        headers: {
          "Set-Cookie": `token=${response.data.jwtToken}; Path=/; Domain=${process.env.NEXT_PUBLIC_COOKIE_DOMAIN}; Secure; SameSite=Lax; Expires=${new Date(Date.now() + 1000 * 60 * 60 * 24).toUTCString()}`,
